...
const Footer = () => {
  return (
    <footer className="bg-white border-t border-slate-100 pt-24 pb-12 px-6">
      ...
      <div className="max-w-7xl mx-auto pt-12 border-t border-slate-100 flex flex-col md:flex-row items-center justify-between gap-6 text-brand-text-secondary/60 text-[10px] font-black uppercase tracking-[0.2em]">
...
const SocialLink = ({ icon, href }) => (
  <a href={href} className="w-11 h-11 bg-slate-50 border border-slate-100 rounded-xl flex items-center justify-center text-brand-text-primary hover:bg-brand-primary hover:text-white hover:border-brand-primary transition-all duration-300">
    {icon}
  </a>
);

const TechBadge = ({ icon, label }) => (
  <div className="flex items-center gap-2 px-4 py-2.5 bg-slate-50 border border-slate-100 rounded-xl text-brand-text-primary font-black text-[10px] uppercase tracking-wider shadow-sm hover:shadow-premium hover:border-brand-primary/20 transition-all cursor-default">
